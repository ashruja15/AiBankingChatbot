...
class ChatbotHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Dataset count (lightweight probe)
        elif path == "/api/dataset/count":
            self.send_response(200)
            self.send_header("Content-type", "application/json")
            self.send_cors_headers()
            self.end_headers()
            self.wfile.write(json.dumps({"count": len(kb)}).encode("utf-8"))
            return
    ...
    def do_POST(self):
        ...
        # Dynamic Q&A addition — POST /api/dataset  {"question": "...", "answer": "..."}
        if path == "/api/dataset":
            ...
            if not question or not answer:
                ...
                self.send_cors_headers()
                self.end_headers()
                self.wfile.write(json.dumps({"error": "question and answer are required"}).encode("utf-8"))
                return

            # Add to in-memory KB
            kb.append({"question": question, "answer": answer})

            # Persist to dataset.json on disk so the change survives reload
